...
<script lang="ts">
    ...
    const { user } = data;
    console.log(user);
    const field: {login_id: string; username: string; password?: string } = { login_id: user.login_id, username: user.username };
    const fieldError : Record<string, string> = {all: '',email: '',login_id: '',username: '',password: ''};
    ...
    const handleSubmitEdit = async (event: Event) => {
        ...
        if (field.login_id === user.login_id) formData.login_id = '';
...
<form class="space-y-4 md:space-y-6" method="post" on:submit|preventDefault={handleSubmitEdit}>
    ...
    <div>
        ...
        <input type="input" name="login_id" id="login_id" bind:value={field.login_id} class="input block w-full p-2.5">
    </div>
    ...
    <div>
        ...
        <input type="input" name="username" id="username" bind:value={field.username} class="input block w-full p-2.5">
    </div>
    ...
    <div>
        ...
        <input type="password" name="password" id="password" bind:value={field.password} placeholder="••••••••" class="input block w-full p-2.5">
    </div>
    ...
    <button on:click={handleSubmitEdit} class="btn variant-ghost-primary w-full font-medium rounded-lg text-sm px-5 py-2.5 text-center">수정 완료</button>
    <button on:click={onClickDelete} class="btn variant-ghost-error w-full font-medium rounded-lg text-sm px-5 py-2.5 text-center">회원 탈퇴</button>
</form>
